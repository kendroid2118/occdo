"use client";

import Link from "next/link";
import { useActionState } from "react";

import type { ProgramCatalogs } from "@/lib/actions/programs";
import { createServiceDeliveryFormAction } from "@/lib/actions/service-deliveries";
import { SERVICE_DELIVERY_ACTION_ERROR_MESSAGE } from "@/lib/programs/delivery-errors";
import { Button } from "@/components/ui/button";

const fieldClass =
  "h-10 w-full rounded-md border border-slate-300 bg-white px-3 text-sm text-slate-900 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-occdo-700";

export type DeliveryCooperativeOption = {
  id: string;
  name: string;
  cooperativeCode: string;
};

type DeliveryFormProps = {
  catalogs: ProgramCatalogs;
  cooperatives: DeliveryCooperativeOption[];
  defaultCooperativeId?: string;
};

export function DeliveryForm({
  catalogs,
  cooperatives,
  defaultCooperativeId,
}: DeliveryFormProps) {
  const [state, formAction, pending] = useActionState(
    createServiceDeliveryFormAction,
    null,
  );
  const catalogsReady = catalogs.serviceTypes.length > 0 && cooperatives.length > 0;
  const errorMessage =
    state && state.ok === false ? SERVICE_DELIVERY_ACTION_ERROR_MESSAGE[state.code] : null;

  return (
    <form action={formAction} className="max-w-3xl space-y-6">
      {!catalogsReady ? (
        <p className="rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-900">
          Active service types and at least one cooperative are required before recording a
          delivery.
        </p>
      ) : null}

      {errorMessage ? (
        <p className="text-sm text-red-700" role="alert">
          {errorMessage}
        </p>
      ) : null}

      <div className="grid gap-4 md:grid-cols-2">
        <div className="space-y-1.5 md:col-span-2">
          <label className="block text-sm font-medium text-slate-800" htmlFor="cooperativeId">
            Cooperative
          </label>
          <select
            className={fieldClass}
            defaultValue={defaultCooperativeId ?? ""}
            id="cooperativeId"
            name="cooperativeId"
            required
          >
            <option value="">Select a cooperative</option>
            {cooperatives.map((cooperative) => (
              <option key={cooperative.id} value={cooperative.id}>
                {cooperative.name} ({cooperative.cooperativeCode})
              </option>
            ))}
          </select>
        </div>
        <div className="space-y-1.5">
          <label className="block text-sm font-medium text-slate-800" htmlFor="serviceTypeId">
            Service type
          </label>
          <select
            className={fieldClass}
            defaultValue=""
            id="serviceTypeId"
            name="serviceTypeId"
            required
          >
            <option value="">Select a service type</option>
            {catalogs.serviceTypes.map((option) => (
              <option key={option.id} value={option.id}>
                {option.name}
              </option>
            ))}
          </select>
        </div>
        <div className="space-y-1.5">
          <label className="block text-sm font-medium text-slate-800" htmlFor="programId">
            Program
          </label>
          <select className={fieldClass} defaultValue="" id="programId" name="programId">
            <option value="">None</option>
            {catalogs.programs.map((option) => (
              <option key={option.id} value={option.id}>
                {option.name}
              </option>
            ))}
          </select>
        </div>
        <div className="space-y-1.5">
          <label className="block text-sm font-medium text-slate-800" htmlFor="deliveredAt">
            Date delivered
          </label>
          <input className={fieldClass} id="deliveredAt" name="deliveredAt" required type="date" />
        </div>
        <div className="space-y-1.5 md:col-span-2">
          <label className="block text-sm font-medium text-slate-800" htmlFor="remarks">
            Remarks
          </label>
          <textarea
            className="min-h-24 w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-occdo-700"
            id="remarks"
            name="remarks"
          />
        </div>
      </div>

      <div className="flex gap-2">
        <Button disabled={pending || !catalogsReady} type="submit">
          {pending ? "Saving…" : "Record delivery"}
        </Button>
        <Button asChild variant="outline">
          <Link href="/programs">Cancel</Link>
        </Button>
      </div>
    </form>
  );
}
