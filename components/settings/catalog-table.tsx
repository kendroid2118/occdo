...
import type { CatalogAdminRecord } from "@/lib/actions/catalog-admin";
import { CATALOG_KIND_META, type CatalogKind } from "@/lib/settings/access";

type CatalogTableProps = {
  kind: CatalogKind;
  items: CatalogAdminRecord[];
};

export function CatalogTable({ kind, items }: CatalogTableProps) {
  ...
  if (items.length === 0) {
    return (
      <p className="rounded-lg border border-dashed border-slate-300 bg-white px-4 py-10 text-center text-sm text-slate-600">
        No {meta.label.toLowerCase()} are listed yet.
      </p>
    );
  }
        ...
        <tbody>
          {items.map((row) => (
            <tr className="border-b border-slate-100 last:border-0" key={row.id}>
              ...
              <td className="px-4 py-3 text-slate-700">{row.isActive ? "Active" : "Inactive"}</td>
              <td className="px-4 py-3">
                <Link
                  ...
                  href={`/settings/reference/${meta.slug}/${row.id}/edit`}
                >
                  Edit
                </Link>
              </td>
